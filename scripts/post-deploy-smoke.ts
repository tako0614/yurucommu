...
export async function main(): Promise<void> {
  ...
  try {
    ...
    if (loginResponse.status !== 200) {
      throw new Error(`POST /api/auth/login returned ${loginResponse.status}`);
    }
    ...
    noteContent = `e2e-note-${crypto.randomUUID()}`;
    ...
    if (
      !isRecord(createdNote.note) ||
      createdNote.note.content !== noteContent
    ) {
  ...
  if (postId) {
    try {
      ...
      await verifyPostAbsent(postId, (path) => requestResponse(path));
  ...
  if (noteCreated) {
    try {
      ...
      const remainingNotes = await requestJson("/api/notes", {
        expectedStatus: 200,
      });
      assertNoteAbsent(remainingNotes, noteContent);
      checks.push("notes.cleanup");
      checks.push("notes.cleanup.readback");
    } catch (error) {
      cleanupErrors.push(error);
    }
  }
...
export function assertNoteAbsent(value: unknown, marker: string): void {
  if (!isRecord(value) || !Array.isArray(value.notes)) {
    throw new Error("notes cleanup readback did not return notes");
  }
  if (value.notes.some((note) => isRecord(note) && note.content === marker)) {
    throw new Error("notes cleanup readback still found the probe note");
  }
}
...
async function requestResponse(
  ...
  return fetch(new URL(path, origin), {
    ...
    headers:
      options.headers ?? (bodyPresent ? jsonHeaders() : requestHeaders()),
...
async function requestJson(
  ...
  if (response.status !== options.expectedStatus) {
    throw new Error(
      `${method} ${path} returned ${response.status}, expected ${options.expectedStatus}`,
    );
  }
